...
// data = a function returning the content (actually HTML)
	...
	Vue.component('milestone', {
		...
		<td v-if="tmp[layer].milestones && tmp[layer].milestones[data]!== undefined && milestoneShown(layer, data) && tmp[layer].milestones[data].unlocked" v-bind:style="[tmp[layer].milestones[data].style]" v-bind:class="{milestone: !hasMilestone(layer, data), tooltipBox: true, milestoneDone: hasMilestone(layer, data), milestoneOverpowered: milestoneOverpowered(layer, data)}">
			<h3 v-html="tmp[layer].milestones[data].requirementDescription + (milestoneOverpowered(layer, data) ? ' (已超强化)':'')"></h3><br>
	...
	Vue.component('resource-display', {
		...
		<div style="margin-top: -13px">
			...
			<span v-if="tmp[layer].passiveGeneration"><br>你正在获得 {{format(tmp[layer].resetGain.times(tmp[layer].passiveGeneration))}} {{tmp[layer].resource}} per second</span>
			<br><br>
			<span v-if="tmp[layer].showBest">你最多拥有 {{tmp[layer].resource}} 数量为 {{formatWhole(player[layer].best)}}<br></span>
	...
	Vue.component('sell-one', {
		...
		template: `
			<button v-if="tmp[layer].buyables && tmp[layer].buyables[data].sellOne && !(tmp[layer].buyables[data].canSellOne !== undefined && tmp[layer].buyables[data].canSellOne == false)" v-on:click="run(tmp[layer].buyables[data].sellOne, tmp[layer].buyables[data])"
				v-bind:class="{ longUpg: true, can: player[layer].unlocked, locked: !player[layer].unlocked }">{{tmp[layer].buyables.sellOneText ? tmp[layer].buyables.sellOneText : "出售1级"}}</button>
	`
	})
